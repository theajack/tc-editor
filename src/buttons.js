...
import {setActiveLineDefTop} from './activeLine';
import {setLinePT} from './line';
...
export function initButtons () {
    ...
    if (this.config.buttons !== false) {
        ...
        var arr = [];
        if (btn === 'all' || btn === true) { // 全部按钮都要
            for (let attr in buttons) {
            ...
            btn.forEach((item) => {
                if (!buttons[item]) {throw new Error('buttons 名称有误：' + item);}
                arr.push(createButton(item, buttons[item], this));
            });
        }
